...
4. **API** — FastAPI BFF com 15 roteiros consulta o schema `publication` e retorna dados prontos para a interface.
...
- FastAPI com pool de conexões psycopg (assíncrono)
- 15 roteiros: competições, partidas, jogadores, times, rankings, mercado, analytics, Copa do Mundo, health, search
...
```
├── api/                 # FastAPI BFF (15 roteiros, schemas, testes)
...
├── data/                # Assets visuais, snapshots
├── tools/               # Release gates, scripts de utilidade
├── src/                 # Biblioteca compartilhada (legado)
├── docs/                # Documentação técnica (38+ arquivos)
