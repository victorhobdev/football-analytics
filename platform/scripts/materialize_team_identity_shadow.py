...
def materialize() -> dict[str, int]:
    ...
    if len(canonical_members) != 1931:
        raise RuntimeError(f"expected corrected 1931 canonical identities, got {len(canonical_members)}")
    ...
    with psycopg.connect(dsn(), row_factory=dict_row) as conn:
        ...
        conn.execute(f"insert into {SCHEMA}.dim_team select canonical_team_id,md5(canonical_team_id::text),team_name,country_or_territory,team_type,gender,category,identity_state,merged_into_team_id from {SCHEMA}.canonical_team order by canonical_team_id")
