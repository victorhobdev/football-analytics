...
external_candidates as (
    select
        ...
        bm.ingested_at,
        'name:' || lower(regexp_replace(trim(bx.home_team_name_raw), '[^[:alnum:]]+', '_', 'g')) as home_source_team_key,
        'name:' || lower(regexp_replace(trim(bx.away_team_name_raw), '[^[:alnum:]]+', '_', 'g')) as away_source_team_key
    ...
    select
        ...
        tg.ingested_at,
        coalesce(
            'club:' || nullif(trim(tg.home_club_id), ''),
            'name:' || lower(regexp_replace(trim(tx.home_team_name_raw), '[^[:alnum:]]+', '_', 'g'))
        ),
        coalesce(
            'club:' || nullif(trim(tg.away_club_id), ''),
            'name:' || lower(regexp_replace(trim(tx.away_team_name_raw), '[^[:alnum:]]+', '_', 'g'))
        )
    from control.external_match_publication_xref px
    ...
    select
        ...
        em.ingested_at,
        'name:' || lower(regexp_replace(trim(ex.home_team_name_raw), '[^[:alnum:]]+', '_', 'g')),
        'name:' || lower(regexp_replace(trim(ex.away_team_name_raw), '[^[:alnum:]]+', '_', 'g'))
    from control.external_match_publication_xref px
...
resolved as (
    ...
    join raw.provider_entity_map hm
     ...
     and hm.source_id = e.home_source_team_key
    join raw.provider_entity_map am
      on am.provider = e.provider
     and am.entity_type = 'team'
     and am.source_id = e.away_source_team_key
    join control.team_identity home_identity
    ...
    join control.team_identity away_identity
