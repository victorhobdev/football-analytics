...
O número anterior de **2.733** era apenas `3.060 - 327`, isto é, o resultado parcial dos componentes detectados por sobreposição de partidas. Os 327 não eram “casos restantes”; eram IDs excedentes já detectados por uma única técnica. A contagem de 1.930 substitui esse resultado parcial.

Em 2026-07-16, a validação dos fatos filhos revelou 122 eventos de 13 partidas da Série B de 2024 associados ao legado `6188` (`CRB`), enquanto os mesmos lados de `raw.fixtures` estavam rotulados como `275822` (`CRB B`). As partidas oficiais CRB × Chapecoense, Botafogo-SP, América-MG e outras dez são do time principal. A união pesquisada `6188 ↔ 275822` foi incorporada ao manifesto; as duas origens agora apontam para `3000000000993` (`CRB`) e os eventos sem lado canônico caíram de 122 para zero. Esse fato corrige a cardinalidade anterior de 1.931 para 1.930 sem alterar a quantidade de partidas canônicas.
...
O resultado de 1.930 é a cardinalidade **materializada em sombra**, não um cutover ativo:
...
- `shadow_team_identity_20260715.canonical_team` contém 1.930 IDs novos;
...
2. Alocar **1.930 IDs internos novos**, sem derivá-los de nome, fonte, competição ou temporada. O número 1.933 foi rejeitado pela divergência Belenenses e pela correção CRB documentadas acima.
...
- `count(*)` do cadastro canônico de clubes = **1.930**;
...
Não existe um identificador mundial único que cubra todas as fontes e toda a história do futebol. O número 1.930 é a contagem de alta confiança para o acervo atual, obtida com regras conservadoras e decisões externas documentadas. Novas fontes ou partidas podem revelar outra cisão histórica ou outro alias; por isso o modelo final precisa aceitar `merged`, decisões negativas e vínculos contextuais sem trocar o ID canônico já publicado.
