...
def _fetch_competitions() -> list[dict[str, Any]]:
    rows = db_client.fetch_all(
        """
        with provider_season_counts as (
            select
                competition_key,
                season_label,
                provider,
        ...
        preferred_seasons as (
            select competition_key, season_label, matches_count
            from (
                select
                    psc.*,
                    row_number() over (
                ...
                from provider_season_counts psc
            ) ranked
        ...
        select
            ...
            competition_key,
            competition_name,
            coalesce(pd.matches_count, rc.matches_count) as matches_count,
            coalesce(pd.seasons_count, rc.seasons_count) as seasons_count,
            coalesce(pd.min_season_label, rc.min_season_label) as min_season_label,
            coalesce(pd.max_season_label, rc.max_season_label) as max_season_label,
            match_statistics_count,
            lineups_count,
            events_count,
            player_statistics_count
        from ranked_competitions rc
        left join preferred_depth pd using (competition_key)
